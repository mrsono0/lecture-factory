Manus AI를 이용하여 04_SlidePrompt의 프롬프트 파일로 슬라이드를 생성합니다.
...
3. **프로젝트 폴더 결정**:
   ...
   - `04_SlidePrompt/` 폴더가 존재하는지 확인합니다.

4. **프롬프트 파일 목록 확인**:
   - `04_SlidePrompt/*슬라이드 생성 프롬프트.md` 패턴으로 파일을 탐색합니다.
...
P04 프롬프트 파일이 대용량(860~1,950줄)인 경우, Manus AI의 최적 처리를 위해 교시(세션) 단위로 자동 분할합니다:
