...
- 대형 교안(3000줄 초과)이라도 **전문 삽입**합니다. 분할 처리는 P07(Manus Slide)의 `split_by_session()`이 교시 단위로 자동 수행합니다.
- **삽입 형식**:
  ```markdown
  ### 교안 원문
  아래의 교안 원문 전체를 참조하여 슬라이드를 생성하세요.
...
> 항상 교안 원문 전문을 삽입하고, 대형 프롬프트의 분할은 P07 파이프라인(D2 Chunk Splitter + `manus_slide.py split_by_session()`)이 전담합니다.
