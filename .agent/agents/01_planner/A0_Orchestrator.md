...
3. **카테고리 결정**: `.agent/AGENTS.md` §Per-Agent Model Routing 테이블에서 에이전트별 카테고리를 조회합니다. 아래 '에이전트별 category→model 매핑' 인라인 테이블로도 동일하게 참조 가능합니다.
4. **model 매핑**: 카테고리 → 모델 변환은 `.opencode/oh-my-opencode.jsonc`의 `categories` 섹션 참조.
