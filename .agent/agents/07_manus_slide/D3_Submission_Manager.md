...
### 1. 제출 전 환경 확인
- `MANUS_API_KEY` 환경변수 확인 (없으면 `.agent/.env` 확인)
...
### 로깅 명령어 템플릿
> `agent_logger.py` CLI를 사용하면 JSONL 수동 구성 없이 정확한 로그를 기록할 수 있습니다. model 필드 자동 매핑, 중복 END 방지, duration 자동 계산이 포함됩니다.
**START (API 호출 직전)**:
```bash
EXT_KEY=$(python3 .agent/scripts/agent_logger.py external-tool-start \
  --workflow 07_Manus_Slide --run-id $RUN_ID \
  --step-id step_4_manus_submission --agent D3_Submission_Manager \
  --category fast-task --action submit_task \
  --tool-name manus-ai --tool-action {create_project|upload_file|create_task})
```
**END (API 호출 완료 후)**:
```bash
python3 .agent/scripts/agent_logger.py external-tool-end \
  ...
  --category fast-task --action submit_task \
  --tool-name manus-ai --tool-action {create_project|upload_file|create_task} \
  --ext-key $EXT_KEY --output-bytes {응답바이트수} \
  --tool-status {success|error} --tool-error "{에러메시지}"
```
