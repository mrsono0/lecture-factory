...
### 문서 구조 검증
- [ ] **청크 타입별 4~7개 섹션 구조**: lab/code는 §1~§7 전부, narrative/diagram은 §1,§2,§3,§6 필수 + §4/§5/§7 해당 시를 따르는가?
...
### 📖 교안 본문 vs 🗣️ 강사 대본 비율 검증 (3-Layer 정합성)
- [ ] **본문 존재 확인**: §2, §3의 모든 하위 섹션에 📖 교안 본문(설명체)이 존재하는가? (대본만 있는 섹션이 없는지 확인)
- [ ] **본문 ≥ 대본**: 각 세션의 📖 교안 본문 분량이 🗣️ 강사 대본 분량 이상인가? (목표 비율 6:4 이상)
- [ ] **어조 분리**: 교안 본문이 설명체(~입니다, ~됩니다), 강사 대본이 구어체(~해요, ~거예요)로 명확히 구분되는가?
- [ ] **본문 자기완결성**: 🗣️ 대본과 💡 강사 노트를 모두 제거해도 교안 본문만으로 학습 내용이 전달되는가?
- [ ] **대본 보충성**: 🗣️ 강사 대본이 교안 본문에 없는 새로운 기술 정보를 포함하지 않는가? (비유·예시만 추가)
...
- [ ] **Step 완전성**: 모든 LLM step에 대해 START/END 쌍이 존재하는가? (step_5a/step_6 스크립트는 자체 로깅)
