...
이 원칙은 모든 에이전트의 산출물에 적용됩니다. 이를 위해 교안은 **3개의 독립적인 콘텐츠 레이어**로 구성됩니다:

- **📖 교안 본문 (Material Body)**: 학습자가 독립적으로 읽고 참조할 수 있는 구조화된 교육 콘텐츠 (정의, 원리, 구조, 비교, 코드 해설). 설명체(~입니다, ~됩니다)로 작성.
- **🗣️ 강사 대본 (Instructor Script)**: 교안 본문을 수업에서 전달하는 방법을 안내하는 화법 스크립트. 구어체(~해요, ~거예요)로 작성. 비유, 스토리텔링, 일상 예시를 활용하여 본문 내용을 풀어 설명.
- **💡 강사 노트 (Instructor Note)**: 비유 의도, 참고자료 출처, 수업 팁 등 메타 정보.

> ⚠️ **교안 본문이 주(主)이고, 강사 대본은 종(從)입니다.** 본문 없이 대본만 있는 섹션은 불완전합니다. 대본은 본문에 없는 새로운 기술 정보를 포함하지 않으며, 본문 내용을 비유와 구어체로 풀어 설명하는 역할만 합니다.

구체적으로:

1. **왜(Why) → 무엇(What) → 어떻게(How) 구조**: 모든 개념 설명은 "왜 이걸 배우는지" → "이것이 무엇인지" → "어떻게 사용하는지" 순서를 따릅니다. 강사가 "왜 이걸 알아야 하죠?"라는 질문에 교안만으로 답할 수 있어야 합니다.
2. **전환 멘트 및 대본 포함**: 세션·주제 간 연결 문장과 상세 대본을 반드시 포함합니다. 강사가 "자, 다음 주제로 넘어갈게요"라고만 하지 않고, "앞에서 변수가 데이터에 이름표를 붙이는 거라고 했죠? 이제 그 이름표가 붙은 데이터를 여러 개 묶어서 관리하는 방법을 배워볼 거예요"처럼 논리적 연결을 제공합니다. **§2 핵심 개념** 섹션의 각 개념 블록에는 반드시 `🗣️ 강사 대본 (Script):`을, 실습 가이드에는 `🎙️ 실습 가이드 대본:`을 제공합니다. §3 상세 내용 등 나머지 섹션에서는 **📖 교안 본문이 먼저 충분히 서술된 후** 필요 시 보충 대본을 선택적으로 배치합니다.
...
교안 산출물은 레이어별로 어조를 차등 적용합니다:

| 레이어 | 어조 | 예시 |
|--------|------|------|
| 📖 교안 본문 | 설명체 (~입니다, ~됩니다) | "변수는 데이터를 저장하는 이름이 붙은 공간입니다. 값을 할당하면 해당 이름으로 데이터를 참조할 수 있습니다." |
| 🗣️ 강사 대본 | 구어체 (~해요, ~거예요) | "자, 이제 변수를 선언해 볼까요? 변수는 상자에 이름표를 붙여놓는 거예요. 이 상자 안에 데이터를 담는 거죠." |
| 💡 강사 노트 | 간결한 안내체 | "이 비유는 AI 시대의 서사 v3에서 차용. 수강생이 멍한 표정이면 실생활 예시를 추가로 제시." |

- ❌ 나쁜 예 (레이어 구분 없음): 전체가 구어체로만 작성 — "변수가 뭐냐면요, 상자에 이름표를 붙여놓는 거예요."
- ✅ 좋은 예 (레이어 구분): 📖 본문에 정의와 원리 서술 → 🗣️ 대본에서 비유로 풀어 설명
- **실습 지시**는 명확하고 간결한 명령조를 유지하되, 앞뒤로 부드러운 연결 멘트를 추가

## 팀 공통 출력 문서 구조 (Document Structure Standard)
모든 교안은 청크 타입에 따라 4~7개 섹션 구조를 따릅니다:
- **lab / code 청크**: 7섹션 전부 필수
...
- **narrative / diagram 청크**: §1개요, §2핵심개념, §3상세내용, §6요약 필수 + §4/§5/§7은 해당 시에만 포함
...
### Step 1: 입력 데이터 확인
1. `01_Planning/강의구성안.md`의 세션별 `📚 참고자료 매핑` 섹션을 확인합니다.
...
- **Source A (로컬 참고자료)**:
  - 강의구성안의 세션별 학습 목표와 `📚 참고자료 매핑`을 기반으로 세션별 타겟 자료를 추출합니다.
...
- **📚 참고자료 매핑 확인 필수**: 강의구성안의 세션별 `📚 참고자료 매핑` 섹션을 반드시 읽고 교안에 반영하라.
...
- **청크 타입별 4~7섹션 구조 필수**: A4B에게 팀 공통 출력 문서 구조(§45-55)의 청크 타입별 섹션을 반드시 구현하라고 지시합니다.
  - ❌ 자체 Expansion Framework(5단계) 사용 금지
  - ✅ lab/code: §1~§7 전부 필수 | narrative/diagram: §1,§2,§3,§6 필수 + §4/§5/§7 해당 시
...
## Step 4 반일(AM/PM) 블록 순차 실행 절차

> step_4_session_writing은 반일(AM/PM) 블록 단위로 순차 실행합니다.
> 한 번에 Day 전체를 위임하지 않습니다.

### 실행 루프 (MANDATORY)

```
for each Day in [Day 1, Day 2, ..., Day N]:
  for each Period in [AM, PM]:
    1. 해당 반일 블록의 세션 목록 추출 (강의구성안 기준)
    2. A4B에게 해당 블록의 세션만 집필 지시 (pair_size: 2 유지)
       - run_in_background: false (동기 실행 필수)
       - task() 재위임 절대 금지 유지
    3. 체크포인트 검증:
       - pairs/ 폴더에 Part 파일 존재 확인
       - 각 파일 크기 > 10KB
       - 필수 섹션 헤더(§1~§6) 존재 확인
    4. 검증 통과 → 다음 블록으로 진행
...
### 반일 블록 정의
- **AM 블록**: 오전 세션 (일반적으로 세션 4개, 강의구성안 기준)
- **PM 블록**: 오후 세션 (일반적으로 세션 4개, 강의구성안 기준)
- 세션 수는 강의구성안의 Day-Session 경계표에 따라 동적 결정

### 왜 Day 단위가 아닌 반일 단위인가?
- Day 전체 = 8세션 = 40~120분 → 실행 시간 과다
- 반일 = 4세션 = 20~60분 → 관리 가능한 단위
- 체크포인트를 반일마다 두어 조기 실패 감지 가능
...
- 품질 검토 리포트 승인
### AM/PM 분할 출력 정책 (필수)
A3(Curriculum Architect)의 Day-Session 경계표와 워크플로우 YAML의 `output_policy`를 연동하여 최종 산출물 형식을 결정합니다. (반일 용량 = 4세션, 50분×4 + 쉬는시간 10분×3 ≈ 4시간)
| 조건 | 출력 형식 | 파일명 |
|------|----------|---------| 
| 1일 4시간 미만 및 단일 일정 | 단일 파일 | `강의교안_v2.1.md` |
| 1일 4시간 **이상** 또는 다일(2일+) 강의 | 일자별 AM/PM 분할 | `Day{N}_{AM\|PM}_{주제요약}.md` (동적 개수) |

> ⚠️ Day 수와 AM/PM 블록 수는 강의구성안에 따라 동적으로 결정됩니다. 고정 5일 10개가 아닙니다.
...
3. **카테고리 결정**: `.agent/AGENTS.md` §Per-Agent Model Routing 표에서 자신의 파이프라인(P02 Writer)과 에이전트명으로 카테고리를 조회합니다.
...
### sequential_direct 실행 시 (A4B pair_size: 2 순차 집필)

A4B의 `sequential_direct` 모드는 Step-by-Step의 변형입니다:
- 2개 세션을 단위(`pair_size: 2`)로 연속 직접 집필
- task() 재위임 절대 금지 — Write 돈구로 직접 파일 저장
- 각 세션 시작/종료 시 START/END를 개별 기록
**로깅 패턴**:
```
step_4 START → 세션 001~002 START/END → 세션 003~004 START/END → ... → step_4 END
```

### 이 파이프라인의 로깅 설정
- **workflow**: `"02_Material_Writing"`
- **워크플로우 YAML**: `.agent/workflows/02_Material_Writing.yaml`
- **기본 실행 모델**: Step-by-Step (sequential_direct, A4B pair_size: 2)
...
4. **분할 판단**: 강의구성안의 일일 강의 시간을 확인하여 4시간 이상 또는 다일 강의 여부를 판단하고, A3에게 AM/PM 분할 설계를 지시합니다. (세션 단위: 50분, 쉬는 시간: 10분)
5. **3-Source 데이터 확인**: 강의구성안 내 세션별 참고자료 매핑 유무, `참고자료/` 폴더 존재 여부, NotebookLM URL 제공 여부를 확인합니다.
...
```bash
START_TIME=$(date +%s)
echo '{"run_id":"[run_id]","ts":"'$(date -u +%FT%T)'","status":"EXTERNAL_TOOL_START","workflow":"02_Material_Writing","step_id":"step_0_context_analysis","agent":"A0_Orchestrator","category":"orchestration","model":"[model]","action":"pdf_extract","tool_name":"pdf-official","tool_action":"extract","tool_input_bytes":[file_size],"retry":0}' >> ".agent/logs/[DATE]_02_Material_Writing.jsonl"
```
