...
A3(Curriculum Architect)의 "오전/오후 분할 설계"(항목 7) 규칙과 워크플로우 YAML의 `output_policy`를 연동하여 최종 산출물 형식을 결정합니다. (세션 단위: 50분, 쉬는 시간: 10분)

| 조건 | 출력 형식 | 파일명 |
|------|----------|---------| 
| 1일 4시간 미만 및 단일 일정 | 단일 파일 | `강의교안_v2.1.md` |
| 1일 4시간 **이상** 또는 다일(2일+) 강의 | 일자별 AM/PM 분할 | `Day{N}_{AM\|PM}_{주제요약}.md` (예: Day1_AM_환경설정과첫코드.md) |
